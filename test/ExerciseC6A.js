...
contract('ExerciseC6A', async (accounts) => {
  ...
  it('function call is made when multi-party threshold is reached', async () => {
    ...
    let startStatus = await config.exerciseC6A.isLive.call();
    let changeStatus = !startStatus;

    // ACT
    await config.exerciseC6A.setContractStatus(changeStatus, {from: admin1});
    await config.exerciseC6A.setContractStatus(changeStatus, {from: admin2});

    let newStatus = await config.exerciseC6A.isLive.call();
